...
def init(v):
    global dataset_dir
    dataset_dir = f"data/{dataset}_{alpha}_{v}/"
...
def FedAvg(policy, param, rseed, from_ckt=False, Debug=True):
    ...
    if from_ckt:
        ...
        shift_test = np.array(metadata["shift_test"])
    ...
    for t in tqdm(range(epoch, T)):
        ...
        if Debug:
            print(f'{policy}V{param}: train loss:{loss_train[t]:.4f}, train acc: {acc_train[t]:.4f}, test loss: {loss_test[t]:.4f}, test acc: {acc_test[t]:.4f}')
            ...
            for att in range(n_attempts):
                ...
                try:
                    with concurrent.futures.ProcessPoolExecutor(initializer=init, initargs=(v, ), max_workers=nworkers, mp_context=multiprocessing.get_context("fork")) as executor:
                        futures = [executor.submit(FedAvg, policy_list[0], V, rseed, load_ckt) for V in V_list]
                        for future in concurrent.futures.as_completed(futures):
                            results = future.result()
                            policy_name = results.pop("policy_name")
                            print(f"{policy_name} completed")
                            with open(f"results/{dataset}_hypertune/{dataset}_{alpha}_{v}_S{rseed}_{policy_name}.json", "w") as f:
                                json.dump(results, f) 
                    break
