...
def import_data(v):
    train_path = f"data/Federated/CIFAR_alpha05_{v}_train.pck"
    test_path = f"data/Federated/CIFAR_alpha05_{v}_test.pck"
...
def get_client_data(uname):
    return train_data["user_data"][uname]
...
def get_train_prop():
    ...
    return train_dist, sumsz_train, szfrac_train
...
def FedAvg(policy, eps=np.log(4)):
    ...
    # print([w.shape for w in shape_list])
    decay_rate = 0.9992
    stepsize = 0.01
...
def main():
    ...
    for v in run_list:
        ...
        with multiprocessing.Pool(initializer=init, initargs=(v, ), processes=4) as pool:        
            ...
            with open(f"results/CIFAR_alpha05_{v}_epslg4.json", "w") as f:
