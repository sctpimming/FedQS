...
def import_data(v):
    train_path = f"data/Federated/FMNIST_alpha03_{v}_train.pck"
    test_path = f"data/Federated/FMNIST_alpha03_{v}_test.pck"
...
def FedAvg(policy):
    ...
    for t in tqdm(range(T)):
        ...
        for n in participants_set:
            ...
            t_end = time()
            max_time = max(max_time, t_end-t_start)
        t_start = time()
        for idx in range(w_num):
        ...
        for layer in global_model.layers:
            ...
            idx += 2
        t_end = time()
        client_runtime[t] = max_time + (t_end-t_start)
...
def main():
    ...
    for v in run_list:
        ...
        with multiprocessing.Pool(initializer=init, initargs=(v, ), processes=12) as pool:        
            ...
            # with open(f"results/FMNIST_alpha03_{v}.json", "w") as f:
