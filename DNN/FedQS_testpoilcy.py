...
def import_data(v):
    train_path = f"data/Federated/FMNIST_alpha1_{v}_train.pck"
    test_path = f"data/Federated/FMNIST_alpha1_{v}_test.pck"
...
def FedAvg(policy):
    ...
    for t in tqdm(range(T)):
        ...
        if policy == "uniform":
            ...
            participants_set, Aq = client_sample_QSH(
                train_dist, test_dist, available_client, m, Aq
        )
        elif policy == "QS":
            participants_set, Aq = client_sample_QS(
                train_dist, test_dist, available_client, m, Aq
            )
        elif policy == "QSPlus":
            participants_set, Aq = client_sample_QSPlus(
                train_dist, test_dist, available_client, m, Aq
        )
        elif policy == "KL":
...
def main():
    ...
    for v in run_list:
        ...
        with multiprocessing.Pool(initializer=init, initargs=(v, ), processes=12) as pool:        
            ...
            with open(f"results/FMNIST_alpha1_{v}_reverse.json", "w") as f:
